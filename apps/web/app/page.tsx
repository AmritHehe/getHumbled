        ...
        <div className="container">
          <div className="max-w-2xl">
            ...
            <div className="flex items-center gap-4">
              <Link href="/contests">
                <button className="px-6 py-3 text-sm font-medium bg-accent text-surface rounded-lg hover:opacity-90 transition-opacity">
                  Browse Contests
                </button>
              </Link>
              <Link href={isAuthenticated ? "/practice/new" : "/auth/signin"}>
                <button className="px-6 py-3 text-sm font-medium text-primary border border-default rounded-lg hover:border-hover transition-colors">
                  Practice with AI
                </button>
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {liveContests.map((contest) => (
                <Link key={contest.id} href={`/contests/${contest.id}`}>
                  <div className="card p-5 hover:border-hover">
      ...
      <section className="section border-t border-default">
        <div className="container">
          <div className="text-center max-w-md mx-auto">
            ...
            {isAuthenticated ? (
              <Link href="/contests">
                <button className="px-8 py-3 text-sm font-medium bg-accent text-surface rounded-lg hover:opacity-90 transition-opacity">
                  Browse Contests
                </button>
              </Link>
            ) : (
